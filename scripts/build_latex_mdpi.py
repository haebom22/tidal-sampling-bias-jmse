...
def main() -> None:
    ...
    body_tex = re.sub(r"\n*(\\begin\{(?:equation|align|eqnarray)\*?\})",
                      r"\n\n\1", body_tex)
    body_tex = re.sub(r"(\\end\{(?:equation|align|eqnarray)\*?\})\n*",
                      r"\1\n\n", body_tex)

    # Wrap each tabular in an adjustbox so an over-wide table (e.g. the 6-column
    # positioning Table 1) is scaled down to the text width instead of running
    # off the right margin; tables that already fit are left untouched.
    body_tex = re.sub(
        r"(\\begin\{tabular\}.*?\\end\{tabular\})",
        r"\\begin{adjustbox}{max width=\\textwidth}\n\1\n\\end{adjustbox}",
        body_tex, flags=re.DOTALL,
    )

    # Long monospace tokens (GEE collection ids like LANDSAT/LC09/C02/T1_L2,
    # API URLs) don't break and overrun the narrow MDPI column. Allow a line
    # break after each '/', '_' and '.' inside \texttt{...} spans.
    def breakable(m: re.Match) -> str:
        inner = m.group(1)
        for ch in ("/", r"\_", "."):
            inner = inner.replace(ch, ch + r"\allowbreak{}")
        return r"\texttt{" + inner + "}"

    body_tex = re.sub(r"\\texttt\{([^{}]*)\}", breakable, body_tex)

    # pandoc glues "No." to the following token with a non-breaking space (~),
    # so a long code like "No.~R26TA01845857-00" can't wrap and overruns the
    # margin. Make that one space breakable so the number can move to the next
    # line. (A blanket digit-hyphen break is avoided: it would corrupt
    # auto-generated \label{...r2-0.98} anchors.)
    body_tex = body_tex.replace("No.~", "No. ")
